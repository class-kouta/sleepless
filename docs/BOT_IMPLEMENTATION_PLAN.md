...
- [x] 本番・ステージングへ `X_BEARER_TOKEN` をCloudflare Secretとして登録する
- [x] `sleepless_counts` migrationを本番・ステージングへ適用する
- [x] 両Workerをデプロイし、Counts取得と動的投稿を確認する
- [ ] 夜間に失効する投稿用Access Tokenを、Phase 5で自動更新する
...
XのOAuth 2.0ユーザーアクセストークンが夜間の投稿中に期限切れになっても、Cron Workerが投稿前に安全に更新し、JST 22:00〜翌06:00の投稿を継続できるようにする。

Phase 4の運用では、JST 1時・2時の投稿後に投稿用Access Tokenが401となり、3時〜6時の投稿が失敗した。Counts API用の `X_BEARER_TOKEN` は継続して利用できており、対象は投稿用のOAuth 2.0ユーザー文脈だけである。

## 9.1 トークンの保存方針

* `X_CLIENT_ID`、`X_CLIENT_SECRET`、トークン暗号化鍵はCloudflare Worker Secretとして管理する。値をGit・ログ・HTTPレスポンス・D1クエリ結果へ出さない。
* 初回だけ、ローカルのOAuth認可フローで取得したRefresh Tokenを、アプリケーション層で暗号化してD1へ登録する。平文のRefresh TokenやAccess TokenをD1へ保存しない。
* D1には暗号文、トークンの有効期限、更新世代、状態、更新leaseの所有者・期限、安全な失敗理由コードを保存する。暗号化鍵はD1へ保存しない。
* XがRefresh Tokenをローテーションして返した場合は、新しいRefresh Tokenの暗号文を同じトランザクションで置き換える。古い値を再利用しない。

## 9.2 投稿前の更新判定

Cronが投稿枠を確保した後、Xへの投稿前にトークン状態を確認する。`ready` の場合だけAccess Tokenの有効期限を確認し、期限が残り5分未満、またはAccess Tokenが未保存ならRefresh Tokenを使って更新する。

```text
bot_runs の投稿枠を確保
    ↓
トークン状態を確認
    ├─ ready → 投稿用Access Tokenの期限を確認
    │             ├─ 5分以上残っている → 現在のAccess Tokenで投稿
    │             └─ 5分未満 / 未保存 → 更新を確保し、更新・保存後に投稿
    ├─ refreshing → 有効なleaseの間だけ上限付きで待機して状態を読み直す
    │                 └─ lease失効 → recovery_requiredへ変更し、投稿を停止
    └─ recovery_required → 更新・投稿を停止
```

更新リクエストは `POST https://api.x.com/2/oauth2/token` を使用し、初期実装では1回の更新確保につき1回だけ送信する。タイムアウト・ネットワーク障害・5xxでは、X側で更新が成功して応答だけを失った可能性があるため、自動再試行せず `recovery_required` とする。成功応答の解析・検証に失敗した場合も同様に扱う。`invalid_grant`・401などの認可エラーでも `recovery_required` とし、投稿せず、トークン値を含まない失敗理由を記録して運用者へ通知する。

別の実行による更新の待機が上限に達した場合は、その投稿枠を失敗として終了する。有効なleaseを奪ったり、追加の更新要求を送ったりしない。D1から状態を確認できない場合も更新・投稿を行わない。

## 9.3 並行実行と失敗時の安全策

投稿枠ごとの `bot_runs` とは別に、`oauth_token_state` でトークン全体の利用可否を管理する。

| 状態 | 意味 |
| --- | --- |
| `ready` | トークンを利用・更新できる |
| `refreshing` | 更新要求を実行中。ほかの実行はトークンを利用・更新しない |
| `recovery_required` | 自動利用を停止し、運用者の復旧を待つ |

更新要求の前に、読み取った世代番号と `ready` を条件として、`refreshing` への変更と一意なlease所有者・期限を原子的に保存する。この確保の成功を確認できた実行だけがRefresh Tokenを使う。確保結果が不明な場合は更新APIを呼ばない。

更新成功時は、Access TokenとRefresh Tokenの暗号文、有効期限、次の世代番号、`ready` への変更、leaseの解除を同じトランザクションで保存する。Refresh Tokenが返らなかった場合は既存の暗号文を維持する。保存条件は `refreshing`、確保時の世代番号、lease所有者、有効なleaseが一致することとし、保存成功を確認してから投稿する。条件が一致しない古い実行は結果を破棄し、投稿しない。

`refreshing` のままleaseが失効した場合は、更新APIがすでに成功した可能性があるため、別の実行が更新を引き継ぐことは禁止する。対象の状態・世代番号・lease所有者・期限を条件に `recovery_required` へ原子的に変更する。Workerが更新要求の前後で停止した場合も、この規則で自動再利用を防ぐ。

更新APIの成功後にD1への保存が失敗した、または保存結果を確認できない場合は投稿を中止する。同じ世代の `refreshing` が残っていれば、所有者を照合して `recovery_required` に変更する。D1障害でこの変更もできなくても、更新要求前に保存した `refreshing` とlease失効時の規則により、後続のCronが古いRefresh Tokenを再利用することを防ぐ。保存が実際には成功していた場合や、すでに復旧されていた場合は、その新しい世代を上書きしない。

停止した投稿枠は `bot_runs` を `failed` として記録して通知する。D1に記録できない場合も安全な構造化ログを残す。`recovery_required` は以後の投稿枠にも適用し、Access Tokenの残存期限にかかわらず更新・投稿を停止する。

復旧は、運用者がローカルのOAuth認可フローをやり直し、専用コマンドで暗号化済みトークンを明示的に初期化する。初期化では既存の世代番号をリセットせず原子的に進め、`ready` への変更とleaseの解除を一括保存する。これにより復旧前の実行が遅れて戻っても保存条件を満たさず、新しいトークン状態を上書きできない。

## 9.4 実装項目

* `oauth_token_state` 用migrationを追加し、状態・世代番号・lease所有者・期限・安全な失敗理由コードを保持する。
* AES-GCMによる暗号化・復号と、暗号化鍵のローテーション手順を実装する。
* `src/x/oauth.ts` に自動再試行を行わないRefresh Token交換処理を追加する。
* `src/tokens.ts` にD1の保存、期限判定、状態遷移、更新lease、世代・所有者による条件付き更新、上限付きの待機を実装する。
* `scheduled` handlerを、投稿前に有効なAccess Tokenを取得する処理へ接続する。
* 初回トークンの暗号化済み投入・緊急時の再初期化を行う、ローカル専用の運用コマンドを追加する。再初期化では世代番号を進め、古い実行の保存を拒否する。
* 更新成功・更新失敗・期限切れ・lease競合・復旧待ちへの移行を、トークン値を含めない構造化ログとD1の実行記録で追跡できるようにする。運用者への通知経路と復旧手順を用意する。
* 応答喪失、D1保存失敗、Worker途中停止、並行実行、復旧後に古い実行が戻るケースを検証する。

## 完了条件

* JST 22:00〜翌06:00の間にAccess Tokenが失効しても、投稿前に自動更新して投稿を継続できる。
* Refresh Token、Access Token、暗号化鍵が平文でGit・ログ・HTTPレスポンス・D1に残らない。
* 同時実行でもRefresh Tokenのローテーション競合を起こさない。
* 認可の再取得が必要な状態では投稿せず、原因を安全に記録して運用者が復旧できる。
* 更新応答を失った場合や更新API成功後にD1保存が失敗した場合、同じRefresh Tokenで自動再試行しない。保存結果が不明でも、新しい世代を上書きしない。
* Workerが途中停止し、`refreshing` のleaseが失効しても、後続のCronは更新を引き継がず、明示的な復旧まで更新・投稿を停止する。
* 並行実行で更新要求を送信するのは確保に成功した1実行だけであり、待機側は保存済みの新しい状態を読み直す。
* 明示的な復旧によって世代番号が進み、復旧前の古い実行が戻ってもトークン状態を上書きしたり投稿したりしない。
...
`apps/web-react` はPhase 6のリリースを止めずにReactを学び、同じ機能を再実装・検証する場所とする。Vanilla版の責務は、React版では次のように対応させる。
